...
logregout <- left_join(logregout,
  ...
  mutate(
    ...
    var = case_when(
      ...
      var == "shf_ntprobnp_catAbove median within EF" ~ "NT-proBNP (pg/ml) >=median vs <median",
      ...
      var == "scb_dispincome_catAbove median within year" ~ "Income >=median vs <median",
      TRUE ~ var
    ),
...
#| fig-cap: "Independent predictors of COPD"
#| fig-width: 6
#| fig-height: 10
...
p <- ggplot(logregout, aes(x = or, y = var2, color = cols)) +
  ...
  scale_x_continuous(trans = "log", breaks = c(0.7, 1, 3), limits = c(0.7, 3)) +
...
gridExtra::grid.arrange(p, t, widths = c(4, 1))
```
...
cn <- c("Variable", "Level", "HFrEF", "HFmrEF", "HFpEF", "p-value\ninteraction")
